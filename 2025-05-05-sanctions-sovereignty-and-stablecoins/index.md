...
Some stablecoin platforms are experimenting with _programmable compliance_ -- smart contracts that can enforce certain rules while [preserving privacy](https://a16zcrypto.com/posts/article/privacy-protecting-regulatory-solutions-using-zero-knowledge-proofs-full-paper/) where appropriate. Others are developing identity systems that let you prove you're not on a [sanctions list](https://www.etonec.com/post/how-to-design-a-compliant-privacy-preserving-fiat-stablecoin-via-zero-knowledge-proofs) without uploading your life history to a database.
...
- Systems that block bad actors but keep working for everyone else.
- Privacy for regular people but transparency for authorities.
- Innovation without enabling new forms of crime.
...
The next time someone asks if stablecoins help sanctioned countries, the honest answer is: sure, but no more than the tools we already have. The better question is: _how do we build financial systems that can achieve our security goals without losing our values of connectivity and inclusion?_ The most effective sanctions might not be the ones that cut people off from the global economy, but the ones that create alternatives so compelling that following the rules becomes the obvious choice.
